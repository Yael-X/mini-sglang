...
FP8 block 量化默认按 `128x128` 组织，但在以下维度不能被 `128*tp` 整除时，现已支持：

- `hidden_size` 对应线性层输出分片不整除。
- `intermediate_size` 对应线性层输入分片不整除。
- `vocab_size` 在 `lm_head/embed_tokens` 的 TP 分片不整除。

实现策略：

1. 反量化使用“主块 + 尾块”路径：
   - 主块区域走 view+broadcast 快路径；
   - 尾行/尾列走局部切片扩展路径。
2. FP8 weight/scale 分片共享同一组 shard 边界，并按 block 边界计算 scale 分片范围，避免错位。

## 已验证配置矩阵

> 下表为静态单测覆盖的关键非整除场景（block=128，tp=3）。

| 场景 | weight 形状 (O, I) | split_dim | 说明 |
|------|-------------------|-----------|------|
| hidden 非整除 | (3073, 769) | 0 | 输出维分片，`3073 % (128*3) != 0` |
| intermediate 非整除 | (769, 5505) | 1 | 输入维分片，`5505 % (128*3) != 0` |
| vocab 非整除 | (151937, 769) | 0 | vocab 维分片，`151937 % (128*3) != 0` |
| 尾块反量化 | (257, 385) | N/A | 同时存在尾行与尾列 |
